...
//				fprintf(stderr, "Trying to find quote: %c\n", quote);
				...
				/*while (isspace(*begin) && begin < end) ++begin;*/ while (((unsigned)*begin > 255 || isspace(*begin)) && begin < end) ++begin;  
				const char *trimmed_end = end - 1;
				/*while (isspace(*trimmed_end) && trimmed_end >= begin) --trimmed_end;*/ while (((unsigned)*trimmed_end > 255 || isspace(*trimmed_end)) && trimmed_end >= begin) --trimmed_end;
			...
			else 
			{
				...
				/*while (*end && !isspace(*end) && *end != '>') end++;*/ while (*end && !((unsigned)*end > 255 || isspace(*end)) && *end != '>') end++;
				val.assign(ptr, end);
				ptr = end;
			}
